...
def test_router_register_ble():
    ...
    router.register_ble("dev_1", "AA:BB:CC:DD:EE:FF")
    assert router._ble_addresses["dev_1"] == "AA:BB:CC:DD:EE:FF"


# ---------- ring bell / beep: unsupported, no network ----------


@pytest.mark.asyncio
async def test_ring_bell_unsupported_without_network():
    """ring_bell returns an explicit unsupported ControlResult with no HTTP."""
    c = make_client()
    calls = []
    ...
    c._request = fail_request
    c._post = fail_request
    result = await c.ring_bell("d1")
...
@pytest.mark.asyncio
async def test_beep_unsupported_without_network():
    """beep returns an explicit unsupported ControlResult with no HTTP."""
    c = make_client()
    calls = []
...
async def test_coordinator_sync_ble_records_simulation():
    ...
    dev = Device(
        device_id="d1",
        name="Smart Lock",
        capabilities={"bleMac": "AA:BB:CC:DD:EE:FF", "deviceBindUserId": "bind123"},
    )
