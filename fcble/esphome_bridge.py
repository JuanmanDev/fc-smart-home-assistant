...
# Configured via FC_ESPHOME_PROXIES env var (JSON) or local dictionary
PROXIES: dict[str, tuple[str, int, Optional[str]]] = {}
_env_proxies = os.environ.get("FC_ESPHOME_PROXIES")
if _env_proxies:
    try:
        import json
        for k, v in json.loads(_env_proxies).items():
            PROXIES[k] = (v[0], int(v[1]), v[2] if len(v) > 2 else None)
    except Exception as err:
        _LOGGER.warning("Could not parse FC_ESPHOME_PROXIES: %s", err)
