...
class FcClient:
    ...
    async def ring_bell(self, device_id: str) -> ControlResult:
        return ControlResult(
            success=False,
            message="Remote ring bell is unsupported: no physical ring command is verified in the FC cloud protocol",
        )

    async def beep(self, device_id: str) -> ControlResult:
        return ControlResult(
            success=False,
            message="Remote beep/locate is unsupported: no physical beep command is verified in the FC cloud protocol",
        )
