...
def _normalize_mac(address: str) -> str:
    """Cloud payloads use raw hex ('AABBCCDDEEFF'); bleak uses
    colon-separated ('AA:BB:CC:DD:EE:FF'). Return the colon form."""
...
class FcTransportRouter:
    ...
    def register_ble(self, device_id: str, address: str) -> None:
        ...
        if not hasattr(self, "_ble_addresses"):
            self._ble_addresses = {}
        # normalize: cloud payloads carry raw hex (AABBCCDDEEFF) while
        # bleak/HA use colon format (AA:BB:CC:DD:EE:FF)
