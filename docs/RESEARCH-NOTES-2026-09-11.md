...
- ESP32 proxies (living-cover C8:F0:9E:XX:XX:01 @ -97dBm, kitchen @ -99)
...
- Two ESP32s act as BT proxies and SEE the lock: living-cover
  (C8:F0:9E:XX:XX:01) rssi -96/-97, kitchen (C8:F0:9E:XX:XX:02) -99.
...
real lock (device `<device-uuid>`, L5-WIFI-QINGKE,
BLE MAC `34:17:27:XX:XX:XX`), and the production Home Assistant instance.
...
| HA instance | Docker `homeassistant` on host `<ha-host-ip>` (Proxmox VM "HomeAssistant-VM", root SSH works) | works |
| HA config path | `/docker/homeassistant` on host (mounted as `/config` in container) — Windows SMB share `\\<storage-host>\docker\homeassistant` | works |
| HA API | `agy --dangerously-skip-permissions --print` + its `home-assistant` MCP (ha.example.duckdns.org, entry loaded, v1.0.0 via HACS) | works |
| Deploy code | `scp` to `root@<ha-host-ip>:/docker/homeassistant/custom_components/fc_smarthome/` then `docker restart homeassistant` | works |
| HA reload without full restart | reload entry via HA API/MCP (`homeassistant.config_entries` reload) or `docker restart homeassistant` | works |

- HA is **HA Container** (no Supervisor), so no SSH add-on; use docker exec
  on `<ha-host-ip>` instead. `docker exec homeassistant python3 ...` works.
...
- `dynamicKey`/`noNetPasswordKey`: `<dynamicKey>`
- `wifissid: "IoT_SSID"` — lock is on a **separate IoT VLAN/SSID**
- `deviceBindUserId`: `<bindUserId>`
...
- HA host sees LAN `192.168.X.0/24`.
- Full TCP port scan (1-65535) of every live host in IoT subnet: only `<ip-1>:80`
  (an ESPHome device) and `<ip-2>:8081` (tproxy, no HTTP response) are open.
  **No lock listening ports (8060/9999/8666/5683) found anywhere.**
- arp-scan: no device with the lock's MAC OUI `34:17:27` on WiFi.
  → **The lock's WiFi is either asleep or on an SSID/VLAN not bridged to
  the HA host** (SSID `IoT_SSID`; the router may isolate it). Only
  Espressif devices seen: `<ip-1>` (MAC `c8:f0:9e:xx:xx:01`) and
  `<ip-2>` (`3c:61:05:xx:xx:02`) — neither is the lock (lock BLE MAC
  `34:17:27:XX:XX:XX`; WiFi MAC may differ — 34:17:27 OUI belongs to the
...
- Conclusion so far: **cloud openLock is genuinely broken/blocked server-
  ...
  paths left: (1) find which VLAN `IoT_SSID` maps to and get the HA host
...
1. `sensor.*_last_unlock_user/method/time` stay `unknown` forever with
   ...
   timestamp). The cloud history HAS user names ("User 1", "User 2",
   "Family", "Guest") and method hints in messageKey.
...
- `%TEMP%\opencode\fc_unlock_probe*.py` —
...
- Mitm capture (from 2026-09-08 session, reused):
  `%TEMP%\opencode\mitm\fc_traffic.jsonl`
...
5. Ask user about `IoT_SSID` VLAN / whether HA can reach it (or get an
   ESPHome BT proxy near the lock for the BLE channel).

## Session update 2026-09-12 ~01:10 (UTC+2) — FIXES DEPLOYED & VERIFIED

All entity bugs fixed, deployed to HA (scp to root@<ha-host-ip>:/docker/
...
| last_unlock_user/method/time | unknown | `User 1` / `finger` / `00:51:27Z` ✓ |
...
`scp -r custom_components/fc_smarthome/* root@<ha-host-ip>:/docker/
...
- User CONFIRMED: no successful remote unlock has EVER happened tonight —
  ...
  18:37:52 "User 1" cloud event was a FINGERPRINT unlock at the door, not
  remote.
...
- Awake-window watcher script kept at
  `%TEMP%\opencode\fc_awake_window_test.py`
