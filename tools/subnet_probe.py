...
async def main() -> None:
    import os
    subnet = sys.argv[1] if len(sys.argv) > 1 else os.environ.get("FC_SUBNET", "192.168.1")
    ...
    gw_ip = f"{subnet}.1"
    print(f"\n== gateway {gw_ip} deep dive ==")
    gw_ports = tcp_ports(gw_ip, [21, 22, 23, 53, 80, 443, 1900, 5353, 5683, 8080, 8081, 8443, 9999])
    print(f"  ports: {gw_ports}")
    for p in (80, 8080):
        try:
            s = socket.create_connection((gw_ip, p), timeout=2)
