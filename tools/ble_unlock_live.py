...
A) Inside the HA container (proxies via our standalone bridge â€” no HA
   manager needed, works with docker exec):
     ...
     # from the dev PC:
     scp -r fcble root@<ha-ip>:/config/python_client/
     scp tools/ble_unlock_live.py root@<ha-ip>:/config/python_client/
     ssh root@<ha-ip>
...
PHASES (run in order; each is safe to repeat):
    ...
    openid N  - handshake + unlock credited to user id N (default 1)

Env/args:
    --mac AA:BB:CC:DD:EE:FF    lock BLE MAC (or set FC_LOCK_MAC)
    --lockid <deviceuuid>       cloud lock id (deviceuuid)
    --key 4CAD...               bluetoothKey (32 hex chars)
    --wait SECONDS              stay connected in 'connect' phase
    --proxy <proxy_name>        force a specific proxy
...
LOCK_MAC = os.environ.get("FC_LOCK_MAC", "AA:BB:CC:DD:EE:FF")
LOCK_ID = os.environ.get("FC_LOCK_ID", "")
...
def main() -> int:
    ...
    if args.phase == "openid":
        return asyncio.run(phase_open(args, 1))  # Slot 1
    return 1
